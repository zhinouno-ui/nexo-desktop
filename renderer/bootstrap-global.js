(() => {
    ...
    let bootstrapReadyEmitted = false;
    let retries = 0;
    const maxRetries = 10;
    const retryMs = 500;
    const requiredChecks = () => ({
        AppState: !!window.AppState,
        elements: !!(window.elements || window.NexoElements),
        refreshProfilesUI: typeof window.refreshProfilesUI === 'function',
        restoreShiftModeMemory: typeof window.restoreShiftModeMemory === 'function',
        initSyncManager: typeof window.initSyncManager === 'function',
        ensureUploadUnlocked: typeof window.ensureUploadUnlocked === 'function' || typeof window.NexoAdminAuth?.ensureUploadUnlocked === 'function'
    });

    const maybeEmitBootstrapReady = () => {
        if (bootstrapReadyEmitted) return true;
        const checks = requiredChecks();
        const ready = Object.values(checks).every(Boolean);
        if (!ready) return false;
        bootstrapReadyEmitted = true;
        window.dispatchEvent(new CustomEvent('nexo-bootstrap-ready'));
        return true;
    };

    window.__nexoBootstrapReady = new Promise((resolve) => {
        const poll = setInterval(() => {
            retries += 1;
            if (maybeEmitBootstrapReady()) {
                clearInterval(poll);
                resolve(true);
                return;
            }
            if (retries >= maxRetries) {
                ...
                console.error(`[bootstrap-global] No se completó la carga tras 5s. Faltan: ${missing.join(', ') || 'desconocido'}. Se detiene el chequeo para evitar loops.`);
                resolve(false);
            }
        }, retryMs);

        window.addEventListener('nexo-bootstrap-ready', () => resolve(true), { once: true });
    });
})();
