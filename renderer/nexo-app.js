    (() => {
        ...
        function refreshProfilesUI() {
            ...
            try {
                ...
                if (elements.profileSelect) {
                    const selectOptions = profileList.map((profile) => `<option value="${profile.id}">${profile.name}</option>`).join('');
                    elements.profileSelect.innerHTML = selectOptions;
                    elements.profileSelect.value = AppState.activeProfileId;
                }
                if (elements.profilesList) {
                    if (window.NexoProfilesUI?.buildProfileRows) {
                        elements.profilesList.innerHTML = window.NexoProfilesUI.buildProfileRows(profileList, profileCounts);
                    } else {
                        const profileRows = [];
                            ...
                            try {
                                ...
                                profileRows.push(`
                                    <div class="history-item" style="display:flex;justify-content:space-between;align-items:center;">
                                        <div><strong>${profileName}</strong><div class="history-item-muted">${contactsForProfile} contactos</div></div>
                                        <div style="display:flex;gap:8px;">
                                            <button class="btn" data-profile-action="open" data-profile-id="${profileId}">Abrir</button>
                                            <button class="btn" data-profile-action="rename" data-profile-id="${profileId}">Renombrar</button>${canDelete ? `<button class="btn btn-danger" data-profile-action="delete" data-profile-id="${profileId}">Borrar</button>` : ''}
                                        </div>
                                    </div>
                                `);
                            } catch (profileError) {
                                reportError('refreshProfilesUI:profile-row', profileError, { index });
                            }
                        });
                        elements.profilesList.innerHTML = profileRows.join('');
                    }
                }
                if (elements.splitImportByFileToggle) elements.splitImportByFileToggle.checked = !!AppState.splitImportByFile;
                if (elements.profilesList && !elements.profilesList.dataset.boundActions) {
                    elements.profilesList.dataset.boundActions = "1";
                    elements.profilesList.onclick = (ev) => {
                        ...
                        if (!pid) return;
                        if (action === "open") window.switchProfile(pid);
                        else if (action === "rename") window.renameProfile(pid);
                        else if (action === "delete") window.deleteProfile(pid);
                    };
                }
            } catch (e) {
                console.error('refreshProfilesUI failed', e);
                try { reportError('refreshProfilesUI', e); } catch (_) {}
            } finally {
                AppState.perfStats = AppState.perfStats || {};
                AppState.perfStats.refreshProfilesUIMs = Math.round(performance.now() - t0);
            }
        }

        window.switchProfile = async (profileId) => {
            try {
                setLoadingState(true, 'Cambiando base…', 20, false);
                try { if (typeof flushSaveQueue === 'function') await flushSaveQueue('profile-switch'); } catch (_) {}
                AppState.selectedContacts.clear();
                AppState.searchIndex = { allIds: [], byId: new Map(), byStatus: new Map(), byShift: new Map(), byProfile: new Map(), byPhoneType: new Map(), byOrigin: new Map() };
                ...
                refreshProfilesUI();
                const finish = () => {
                    remountDashboardState('profile-switch');
                    setLoadingState(false, 'Listo', 100, false);
                };
        ...
        window.renameProfile = async (profileId) => {
            const profile = (AppState.profiles || []).find((p) => p.id === profileId);
            if (!profile) return;
            const nextName = normalizeProfileName(window.prompt('Nuevo nombre del perfil', profile.name || '') || '').slice(0, 60);
            if (!nextName) return;
            const duplicated = (AppState.profiles || []).some((p) => p.id !== profileId && normalizeSearchText(p.name) === normalizeSearchText(nextName));
            if (duplicated) {
                showNotification('Ya existe un perfil con ese nombre', 'warning');
                return;
            }
            if (window.electronAPI?.renameProfile) {
                try {
                    const r = await window.electronAPI.renameProfile({ id: profileId, name: nextName });
                    if (r?.ok && Array.isArray(r.profiles)) AppState.profiles = r.profiles;
                    else profile.name = nextName;
                } catch (_) { profile.name = nextName; }
            } else {
            ...
            refreshProfilesUI();
            render();
            showNotification('Perfil renombrado', 'success');
        };
        ...
        window.deleteProfile = async (profileId) => {
            ...
            if (window.electronAPI?.deleteProfile) {
                try {
                    const r = await window.electronAPI.deleteProfile({ id: profileId });
                    if (r?.ok && Array.isArray(r.profiles)) AppState.profiles = r.profiles;
                    else AppState.profiles = AppState.profiles.filter(p => p.id !== profileId);
                } catch (_) {
...
¿Querés forzar igual la instalación?`);
            ...
            if (elements.importFullSnapshotBtn) elements.importFullSnapshotBtn.onclick = importFullSnapshot;

            const runControlExportDaily = async () => {
                if (!window.api?.exportDaily) {
                    showNotification('API de export diario no disponible', 'warning');
                    return null;
                }
                const result = await window.api.exportDaily({ profileId: AppState.activeProfileId || 'default' });
                if (result?.ok) {
                    addToHistory('Export .nexo diario', result.filePath || '');
                    showNotification(`Log diario exportado: ${result.filePath || 'ok'}`, 'success');
                    return result;
                }
                showNotification(`Error export diario: ${result?.message || 'desconocido'}`, 'error');
                return null;
            };

            const runControlExportBackup = async () => {
                if (!window.api?.exportFull) {
                    showNotification('API de export backup no disponible', 'warning');
                    return null;
                }
                const result = await window.api.exportFull({ profileId: AppState.activeProfileId || 'default' });
                if (result?.ok) {
                    addToHistory('Export .nexo backup', result.filePath || '');
                    showNotification(`Backup exportado: ${result.filePath || 'ok'}`, 'success');
                    return result;
                }
                showNotification(`Error export backup: ${result?.message || 'desconocido'}`, 'error');
                return null;
            };

            if (elements.exportDailyNexoBtn) {
                elements.exportDailyNexoBtn.onclick = () => runControlExportDaily();
            }
            if (elements.exportBackupNexoBtn) {
                elements.exportBackupNexoBtn.onclick = () => runControlExportBackup();
            }
            if (elements.metricsShiftMorningBtn) {
                elements.metricsShiftMorningBtn.onclick = () => {
                    AppState.metricsHourRange = { from: 6, to: 14, label: 'TM 06-14' };
                    if (elements.metricsModal?.classList.contains('active')) renderMetricChartsFromEvents();
                };
            }
            if (elements.metricsShiftAfternoonBtn) {
                elements.metricsShiftAfternoonBtn.onclick = () => {
                    AppState.metricsHourRange = { from: 14, to: 22, label: 'TT 14-22' };
                    if (elements.metricsModal?.classList.contains('active')) renderMetricChartsFromEvents();
                };
            }
            if (elements.metricsShiftNightBtn) {
                elements.metricsShiftNightBtn.onclick = () => {
                    AppState.metricsHourRange = { from: 22, to: 6, label: 'TN 22-06' };
                    if (elements.metricsModal?.classList.contains('active')) renderMetricChartsFromEvents();
                };
            }
            if (elements.metricsShiftResetBtn) {
                elements.metricsShiftResetBtn.onclick = () => {
                    AppState.metricsHourRange = null;
                    if (elements.metricsModal?.classList.contains('active')) renderMetricChartsFromEvents();
                };
            }
            if (elements.resetBaselineBtn) {
                elements.resetBaselineBtn.onclick = () => {
                    const profileId = AppState.activeProfileId || 'default';
                    if (!confirm(`¿Resetear baseline mensual del perfil ${profileId}?`)) return;
                    delete AppState.monthlyBaselineByProfile[profileId];
                    savePreferences();
                    showNotification('Baseline mensual reseteado', 'warning');
                };
            }
            if (elements.uploadReportBtn) {
                elements.uploadReportBtn.onclick = async () => {
                    if (!(await ensureUploadUnlocked())) return;
                    const selected = await runControlExportDaily();
                    if (!selected?.ok) {
                        showNotification('No se pudo preparar el log diario', 'error');
                        return;
                    }
                    await queueReportUpload(JSON.stringify({ filePath: selected.filePath || '', exportedAt: new Date().toISOString() }), 'daily-log');
                };
            }
            if (elements.importControlBtn) {
                elements.importControlBtn.onclick = async () => {
                    if (!ensureControlPasswordAccess()) return;
                    try {
                        const files = await window.electronAPI?.openImportDialog?.();
                        const selected = Array.isArray(files) ? files.find((f) => /\.json$/i.test(f)) : null;
                        if (!selected) {
                            showNotification('Seleccioná un archivo JSON de control', 'warning');
                            return;
                        }
                        const text = await window.electronAPI.readTextFile(selected);
                        const parsed = JSON.parse(text);
                        importControlPayload(parsed);
                    } catch (e) {
                        reportError('importControlFile', e);
                        showNotification(`No se pudo importar archivo de control: ${e?.message || e}`, 'error');
                    }
                };
            }
