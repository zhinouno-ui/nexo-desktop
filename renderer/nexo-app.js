    (() => {
        ...
        function safeActivityIso(value) {
            const parsed = new Date(value || Date.now());
            if (!Number.isFinite(parsed.getTime())) return new Date().toISOString();
            return parsed.toISOString();
        }

        function emitShadowActivityLog(payload = {}) {
            try {
                if (!(window.nexoStore && typeof window.nexoStore.logActivity === 'function')) return;
                const normalized = payload && typeof payload === 'object' ? payload : {};
                const eventPayload = {
                    timestamp: safeActivityIso(normalized.timestamp),
                    profileId: String(normalized.profileId || AppState.activeProfileId || 'default') || 'default',
                    contactId: normalized.contactId === undefined || normalized.contactId === null ? null : String(normalized.contactId),
                    action: String(normalized.action || 'unknown'),
                    from: normalized.from === undefined || normalized.from === null ? '' : String(normalized.from),
                    to: normalized.to === undefined || normalized.to === null ? '' : String(normalized.to),
                    shift: normalized.shift === undefined || normalized.shift === null ? '' : String(normalized.shift)
                };
                window.nexoStore.logActivity(eventPayload).catch(() => {});
            } catch (_) {}
        }
        ...
        window.copyToClipboard = (text, event) => {
            ...
            const inferredContactId = cardEl?.getAttribute?.('data-id');
            const contact = inferredContactId ? (AppState.searchIndex?.byId?.get(Number(inferredContactId)) || AppState.searchIndex?.byId?.get(inferredContactId) || AppState.contacts.find((c) => String(c.id) === String(inferredContactId))) : null;
            emitShadowActivityLog({
                timestamp: new Date().toISOString(),
                profileId: contact?.profileId || AppState.activeProfileId || 'default',
                contactId: contact?.id ?? inferredContactId ?? null,
                action: 'copyToClipboard',
                from: '',
                to: '',
                shift: contact?.shiftReviewedByShift || contact?.assignedShift || ''
            });
        ...
        window.changeContactStatus = (id, newStatus, event, source = 'common', meta = {}) => {
            ...
            const safeEventAt = safeActivityIso(eventAt);
            if (oldStatus === requestedStatus) {
                ...
                setSaveState('pending', 'Guardado diferido...');
                enqueueStatusDelta(contact, oldStatus, requestedStatus, safeEventAt);
                emitShadowActivityLog({
                    timestamp: safeEventAt,
                    profileId: contact.profileId || AppState.activeProfileId || 'default',
            ...
            AppState.statusTransitions.unshift({ at: safeEventAt, from: oldStatus, to: requestedStatus, contactId: contact.id, profileId: contact.profileId || 'default', actor: AppState.operatorName || 'PC local', shift: shiftByEvent || contact.shiftReviewedByShift || contact.assignedShift || '' });
            ...
            AppState.buttonPressEvents.unshift({ at: safeEventAt, action: 'status-change', from: oldStatus, to: requestedStatus, shift: shiftByEvent || contact.shiftReviewedByShift || contact.assignedShift || '', profileId: contact.profileId || 'default', actor: AppState.operatorName || 'PC local' });
...
¿Querés forzar igual la instalación?`);
            ...
            const ensureUploadUnlocked = async () => {
                const profileId = AppState.activeProfileId || 'default';
                const uploadUnlock = window.ensureUploadUnlocked || window.NexoAdminAuth?.ensureUploadUnlocked;
                if (uploadUnlock) {
                    return uploadUnlock({
                        profileId,
        ...
        window.addEventListener('nexo-bootstrap-ready', () => {
            init();
        }, { once: true });
    })();
