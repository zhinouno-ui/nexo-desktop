...
function getActivityLogsDir() {
  return path.join(app.getPath('userData'), 'logs');
}

function getActivityLogPath(profileId = 'default') {
  const safeProfileId = String(profileId || 'default').trim().replace(/[^a-z0-9_-]/gi, '_') || 'default';
  return path.join(getActivityLogsDir(), `activity-${safeProfileId}.log`);
}
...
function normalizeActivityPayload(payload) {
  ...
  const profileId = String(source.profileId || 'default').trim() || 'default';
  return {
    timestamp: safeIsoTimestamp(source.timestamp),
    profileId,
    contactId: source.contactId === undefined || source.contactId === null ? null : String(source.contactId),
    action: String(source.action || '').trim() || 'unknown',
    from: source.from === undefined || source.from === null ? '' : String(source.from),
    to: source.to === undefined || source.to === null ? '' : String(source.to),
    shift: source.shift === undefined || source.shift === null ? '' : String(source.shift)
  };
}

async function appendActivityLog(payload) {
  const event = normalizeActivityPayload(payload);
  const out = getActivityLogPath(event.profileId);
  await fs.mkdir(path.dirname(out), { recursive: true });
  await fs.appendFile(out, `${JSON.stringify(event)}\n`, 'utf8');
...
ipcMain.handle('store:logActivity', async (_event, payload) => {
  try {
    return await appendActivityLog(payload);
  } catch (error) {
    await appendErrorLog('store:logActivity', error, { payload });
    return { ok: false, message: error?.message || 'No se pudo registrar actividad' };
  }
});
